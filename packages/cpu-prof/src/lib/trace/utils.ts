...
export function cpuProfilesToTraceFile(
    ...
    if (startTracingInBrowser) {
        ...
        const url =
            mainProfileInfo?.prefix === 'CPU'
                ? 'cpu:profile'
                : 'cpu: ' +
                decodeCmd((mainProfileInfo?.prefix ?? '')?.replace('MAIN-CPU--', ''));
    ...
    allEvents = [
        ...
        ...preparedProfiles.flatMap((profileInfo, index) => {
            ...
            return [
                // startTracingInBrowser already adds a getThreadNameTraceEvent event @TODO ugly logic and separated through this function. clean it up
                ...(startTracingInBrowser && mainPid !== pid && mainTid !== tid ? [getThreadNameTraceEvent(pid, tid, `P:${pid}, T:${tid}`)] : []),
